...
export function DemandeBuilder({
  ...
  const times = useMemo(
    () => (day && duration ? availableStartTimes(day, duration, booked) : []),
    [day, duration, booked],
  );
  ...
  const canSubmit = chosen.length > 0 && day && slot && !pending;
            ...
            <>
              ...
              {day && (
                <div className="mt-4">
                  ...
                  {times.length === 0 ? (
                    <p className="text-sm text-ink-muted">Aucun créneau ce jour-là — essayez une autre date.</p>
                  ) : (
                    <div className="flex flex-wrap gap-2">
                      {times.map((t) => (
                        <button
