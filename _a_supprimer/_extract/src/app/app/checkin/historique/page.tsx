    ...
    <>
      ...
      {checkins.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {checkins.map((c) => (
            <Link key={c.id} href={`/app/checkin/rapport/${c.id}`}>
              <Card className="p-4 transition-colors hover:border-line-gold">
                ...
                <div className="mt-3 flex gap-2 border-t border-line-soft pt-3 text-[11px] text-ink-muted">
                  <span>{c.ref}</span>
                  <span>·</span>
                  <span>{c.damages.length} dommage(s)</span>
                  <span>·</span>
                  <span>{c.photos.length} photo(s)</span>
                  {c.signature && <span>· Signé ✓</span>}
                </div>
              </Card>
            </Link>
          ))}
